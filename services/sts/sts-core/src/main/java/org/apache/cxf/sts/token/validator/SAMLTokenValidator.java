...
public class SAMLTokenValidator implements TokenValidator {
    ...
    protected boolean validateConditions(
        ...
        if (validFrom != null && validFrom.isAfter(Instant.ofEpochMilli(System.currentTimeMillis()))) {
            LOG.log(Level.WARNING, "SAML Token condition not met");
            return false;
        } else if (validTill != null && validTill.isBefore(Instant.ofEpochMilli(System.currentTimeMillis()))) {
            LOG.log(Level.WARNING, "SAML Token condition not met");
            validateTarget.setState(STATE.EXPIRED);
            return false;
        }

        if (issueInstant != null && issueInstant.isAfter(Instant.ofEpochMilli(System.currentTimeMillis()))) {
